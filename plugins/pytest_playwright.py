    ...
    def did_finish_test(self, failed: bool) -> None:
        ...
        if self._tracing_option == "on" or (
            ...
            for index, trace in enumerate(self._traces):
                ...
                # -------------------------------------------------------
                # extra_content: add allure content for more data display.
                # the trace data will show in _artifact_recorder step of Tear Down
                allure.attach.file(
                    source=trace_path,
                    name=os.path.basename(trace_path),
                    attachment_type=None,
                    extension=trace_path.split('.')[-1]
                )
                # -------------------------------------------------------
        ...
        if preserve_video:
            for index, page in enumerate(self._all_pages):
                ...
                try:
                    ...
                    # -------------------------------------------------------
                    # extra_content: add allure content for more data display.
                    # the video data will show in _artifact_recorder step of Tear Down
                    allure.attach.file(
                        source=_build_artifact_test_folder(
                            self._pytestconfig, self._request, video_file_name
                        ),
