...
# TODO 还需要处理动态值
"""
1、获取所有的test-cases文件路径
2、读取文件路径获取所有的测试数据
3、收集所有的测试失败的数据即status为broken和failed的数据
4、
"""
...
class ErrorTestCase:
    ...
    def get_error_case_data(self):
        ...
        @return:
        """
        path = get_all_path(self.test_case_path)
        files = []
        for i in path:
    ...
    def get_case_data(self, test_case):
        ...
        if test_case['testStage']['status'] == 'broken':
            ...
            # 如果是failed的状态则说明是在中间的某个步骤异常导致测试失败，
            # 所以需要获取到的数据是在测试用例执行过程中的testStoge数据
...
class ErrorCaseExcel:
    ...
    def __init__(self):
        ...
        self._file_path = ensure_path_sep("\\file\\" + "自动化异常测试用例1.xlsx")
    ...
    def write_case(self):
        ...
        if len(_data) > 0:
            ...
            self.app.quit()
            # 有数据才发送企业微信
            WeChatSend(AllureDataCollect().get_testcase_count()).send_file_msg(self._file_path)
