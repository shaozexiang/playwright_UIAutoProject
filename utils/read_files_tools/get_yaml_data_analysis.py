...
class CaseDataCheck:
    ...
    def check_params_exit(self):
        # 通过值映射到具体的键上，key返回的是具体的是Enum类对应的值
    ...
    def get_dependence_case_data(self):
        ...
        if _dep_data:
            assert self.case_data.get(TestCaseEnum.DE_CASE_DATA.value[0]) is not None, (
                f"程序中检测到您的 case_id 为 {self.case_id} 的用例存在依赖，但是缺少依赖数据."
        ...
        for key, values in data.items():
            ...
            if key != 'case_common':
                ...
                if case_id_switch is True:
                    try:
                        case_list.append({key: TestCase(**case_date).model_dump()})
                    except Exception as ex:
                        raise KeyError(f"数据格式校验失败，请检查key={key},"
                                       f"异常信息为：{ex}")
                else:
