...
from utils.logging_tools.log_control import ERROR, WARNING, INFO
...
class AssertUtil:
    ...
    def _assert(self, check_value: Any, expect_value: Any, message=None):
        """
        default assertion statement
        """
        if message is None:
            message = f"断言失败，check_value={check_value} data_type={type(check_value)}, expect_value={expect_value}  data_type={type(expect_value)}, assert_type={self.get_type}"
        self.functions_mapping()[self.get_type](check_value, expect_value, str(message))
        # 等价写法
        # self.functions_mapping().get(self.get_type)(check_value, expect_value, str(message))
    ...
    @property
    def _assert_prepare_resp_data(self):
        """
        增加断言后置数据的情况
        """
    ...
    def prepare_assert_handle(self):
        if self.get_assert_type is None:
            INFO.logger.info("正在执行后置处理的前置数据内的断言................")
            self._assert(self._assert_prepare_resp_data, self.get_value,
                         "后置处理断言失败，将不会缓存set_cache中的数据!!")
        else:
            raise AssertTypeError("断言失败，后置处理中param_prepare只支持响应断言")

    def dependent_assert_handle(self):
        if self.get_assert_type is None:
            INFO.logger.info("正在执行后置处理的前置数据内的断言................")
            self._assert(self._assert_prepare_resp_data, self.get_value,
                         "依赖处理断言失败，将不会缓存set_cache中的数据!!")
        else:
            raise AssertTypeError("断言失败，后置处理中param_prepare只支持响应断言")
...
class Assert(AssertUtil):
    ...
    def prepare_assert_handle(self):
        for i in self.assert_data_list():
            self.assert_data = i
            super().prepare_assert_handle()

    def dependent_assert_handle(self):
        for i in self.assert_data_list():
            self.assert_data = i
            super().dependent_assert_handle()
