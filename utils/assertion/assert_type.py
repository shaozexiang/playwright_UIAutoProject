...
def equals(
    ...
    assert check_value == expect_value, ERROR.raise_errorException(message)
...
def length_equals(
    ...
    assert len(check_value) == expect_value, ERROR.raise_errorException(message)
...
def length_greater_than(
    ...
    assert len(str(check_value)) > expect_value, ERROR.raise_errorException(message)
...
def contains(check_value: Any, expect_value: Any, message: Text = ""):
    ...
    assert expect_value in check_value, ERROR.raise_errorException(message)
...
def contained_by(check_value: Any, expect_value: Any, message: Text = ""):
    ...
    assert check_value in expect_value, ERROR.raise_errorException(message)
...
def startswith(
    ...
    assert str(check_value).startswith(str(expect_value)), ERROR.raise_errorException(message)
...
def endswith(
    ...
    assert str(check_value).endswith(str(expect_value)), ERROR.raise_errorException(message)
...
def int_equal(
    ...
    """将数据转化为int类型，再检查是否相等"""
    assert int(check_value) == int(expect_value), message
