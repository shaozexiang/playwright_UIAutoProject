...
from utils.database_tools.mysql_control import MysqlDB
...
class TearDownHandler:
    ...
    def get_cache_name(
        ...
        if "$set_cache{" in replace_key and "}" in replace_key:
            ...
            CacheHandler.update_cache(cache_key=cache_name, cache_value=resp_case_data)
    ...
    def dependent_type_request(
        ...
        """
        try:
            _request_set_value = teardown_case_data.set_cache
            _replace_key = teardown_case_data.replace_key
            ...
            if _request_dependent is not False:
                ...
                if _request_set_value is not None:
                    self.get_cache_name(
                ...
                if _replace_key is not None:
                    data = self.jsonpath_replace_data(
                        replace_key=_replace_key,
                        replace_value=_request_case_data
                    )
                    return data
            else:
                raise JsonpathExtractionFailed(
    ...
    def dependent_self_response(
        ...
        try:
            ...
            if _response_dependent is not False:
                _resp_case_data = _response_dependent[0]
                if teardown_case_data.assert_data is not None:
                    Assert(assert_data=teardown_case_data.assert_data,
                           sql_data=None,
                           request_data=None,
                           response_data=res,
                           status_code='200').prepare_assert_handle()
                CacheHandler.update_cache(cache_key=_set_value, cache_value=_resp_case_data)
                self.get_cache_name(
    ...
    def send_request_handler(
        ...
        """
        后置请求处理
        1、通过case_id对应的yaml来发送相对应的send_request请求
        2、dependent_type对应的是当前用例的数据，拿出当前用例的缓存/响应/请求数据
        替换掉需要发送删除请求的数据最后发送一个删除请求完成数据清理的工作
        :param: resp_data不能对类型进行限制
        @return:
        """
        ...
        for i in _send_request:
            ...
            # 判断请求中的数据
                ...
                exec(self.dependent_type_request(
                    teardown_case_data=i,
    ...
    def prepare_assert_handler(self, param_prepare):
        """
        处理前置请求中的断言数据
        """
        # _assert_data = self._res.yaml_data.assert_data
        #     Assert(assert_data=teardown_case_data.assert_data,
        #            sql_data=None,
        #            request_data=None,
            ...
            for _data in _teardown_data:
                if _data.param_prepare is not None:
                    ...
                    self.send_request_handler(
                        ...
                        # 这里不能预先加载响应数据json.loads(_resp_data)，因为如果使用的是导出接口，响应数据不是一个键值对的形式所以需要将数据处理放入到响应中再进行处理
                        resp_data=_resp_data
                    )
        self.teardown_sql()

    def teardown_sql(self) -> None:
        """处理后置sql这是为了防止出现无法通过任意接口删除掉添加的数据，
        只能直接通过数据库删除的情况
        """
