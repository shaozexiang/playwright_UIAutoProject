...
from utils.database_tools.mysql_control import SetUpMySQL
...
class DependentCase:
    ...
    def url_replace(
        ...
        # 那么可以使用如下方式编写用例, 可以使用 $url_param{}替换,
        # 如/api/v1/work/spu/approval/spuApplyDetails/$url_params{id}
        在replace_key中也需要使用$url_param才能实现$url_param，url上的参数需要对应
            ...
            if config.mysql_db.switch:
                ...
                for i in dependent_data:
                    ...
                    if _set_value is not None:
                        CacheHandler.update_cache(cache_key=_set_value, cache_value=jsonpath_data[0])
    ...
    def dependent_handler(
        ...
        if assert_data is not None:
            Assert(assert_data=assert_data,
                   sql_data=None,
                   request_data=None,
                   response_data=data,
                   status_code='200').dependent_assert_handle()
        if set_value is not None:
            if len(jsonpath_data) > 1:
                CacheHandler.update_cache(cache_key=set_value, cache_value=jsonpath_data)
            else:
                CacheHandler.update_cache(cache_key=set_value, cache_value=jsonpath_data[0])
    ...
    def get_dependent_data(self) -> None:
        ...
        if _dependent_data is not None and _dependent_data is not False:
            for key, value in _dependent_data.items():
                ...
                # jsonpath 数据解析
                # 不要删 这个yaml_case,此处的yaml_case主要是为了修改self.yaml_case中的值
